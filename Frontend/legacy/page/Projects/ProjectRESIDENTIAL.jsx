...
export default function ProjectRESIDENTIAL() {
  const [params, setParams] = useSearchParams();
  const page = Math.max(1, Number(params.get("page") || 1));

  // Lọc dự án theo category TOWNHOUSE (Nhà phố hiện đại) và đảo ngược thứ tự (Z → A)
  const filteredProjects = PROJECTS.filter(
    (p) => p.category === PROJECT_CATEGORIES.TOWNHOUSE
  ).reverse();
  ...
  return (
    <div className="container mx-auto px-4 py-8 md:py-10">
      <h2 className="text-2xl md:text-3xl font-extrabold text-orange-600 mb-6">Nhà Phố Hiện Đại</h2>
